...
# Calculate Manhattan distances for intersections
distances = {c: abs(c[0]) + abs(c[1]) for c in intersections}
closest = min(distances.items(), key=lambda item: item[1])

# Calculate real distances
intersection_distances = {}
# Loop over each route, "bookmarking" all intersections along with the
# distance to it.
for i in range(1, 3):
    distance = 0
    for coord in [c for c, r in grid.items() if r == i or r == 'X']:
        distance += 1
        if coord in intersections:
            if not intersection_distances.get(coord):
                intersection_distances[coord] = {}
            intersection_distances[coord][i] = distance
...
print(f"\nGrid size: {x_max -2} x {y_max - 2}")
print(f"{len(intersections)} intersections")
print(f"Closest intersection distance is {closest}")
